...
This provider isn't listed on `registry.opentofu.org` yet (only
`registry.terraform.io`, where you're reading this). `tofu init` resolves an
unqualified `source` against OpenTofu's own registry first, so qualify the
hostname explicitly:
...
This works today under OpenTofu — signature verification and install both succeed
against a real release, tested directly.
